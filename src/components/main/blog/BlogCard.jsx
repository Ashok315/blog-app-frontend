...
export const BlogCard = ({title,slug,description,blogImage,authorName,authorId,authorImage,createdAt,category}) => {
  ...
  return (
    <>
       <div className='card rounded-lg bg-white dark:bg-slate-700 max-w-[17.8rem] p-[0.7rem] shadow-md'>
            {/* blog image */}
            ...
            <div onClick={()=>navigate(`/blog/${slug}`)} className='blog-content my-4 leading-[1.2rem] cursor-pointer px-2'>
                <p className='blog-title font-bold text-[0.9rem]  text-gray-700 hover:text-black dark:text-gray-200 duration-300'>{title.slice(0,67)}..</p>
            ...
            <div className='flex items-center justify-between gap-2 rounded-md mt-4 px-2'>
                  <div className='flex gap-2 items-center'>
                      ...
                      <div onClick={()=>navigate(`/blogs/author/${authorId}`)} className='leading-4 cursor-pointer'>
                          <p className='author-name text-gray-600 dark:text-gray-300 text-wrap font-semibold text-[0.75rem] capitalize'>{authorName}</p>
                          <p className='created-date text-gray-500 dark:text-gray-200 text-[0.72rem]'>{createdAt}</p>
                      </div>   
                  </div>
                    
                  <button onClick={()=>navigate(`/blogs/${category}`)} className='category-btn border border-lightBorder dark:border-slate-400 text-secondary dark:text-white rounded-md text-[0.72rem] px-[0.55rem] py-[0.1rem] capitalize'>{category}</button>       
            </div>
