...
export const Header = () => {
  ...
  return ( 
      <nav className='bg-white dark:bg-slate-800 py-[0.55rem] fixed top-0 w-full z-[60]'>
        <ContentContainer className='flex gap-3 justify-between items-center'>
          ...
          <ul className='hidden md:flex md:gap-5 content-start lg:ml-6 text-[0.88rem] text-gray-700 dark:text-[#b6b8ba] font-semibold tracking-wide'>
              <li><Link to="/" className={`border-b-2 ${location.pathname=='/'?'border-primary text-black dark:text-white':"border-transparent"} hover:text-black dark:hover:text-white ease-in-out duration-300 pb-1`}>Home</Link></li>
              <li><Link to="/blogs" className={`border-b-2 ${location.pathname=='/blogs'?'border-primary text-black dark:text-white':"border-transparent"} hover:text-black dark:hover:text-white ease-in-out duration-300 pb-1`}>Blogs</Link></li>
              <li><Link to="/about_us" className={`border-b-2 ${location.pathname=='/about_us'?'border-primary text-black dark:text-white':"border-transparent"} hover:text-black dark:hover:text-white ease-in-out duration-300 pb-1`}>About</Link></li>
          </ul>

          {/* themeButton */}
          <div onClick={()=>dispatch(themeToggle())} className='hidden md:block lg:ml-6 border border-gray-500 rounded-full p-[0.33rem] hover:bg-gray-200 dark:hover:bg-gray-700 duration-200 cursor-pointer'>
          ...
          {isAuthenticated?
              ...
              <div ref={profileMenuRef} onClick={toggleProfileTag} className='hidden relative md:flex items-center gap-2 rounded-md py-1 px-4 duration-200 cursor-pointer'>
                  ...
                  <div className={`${!isProfileTabOpen?'hidden':''} absolute bg-white dark:bg-slate-800 w-full py-2 top-10 left-0 rounded-md border border-gray-300 z-10`}>
                      ...
                      <ul className='text-sm text-justify'> 
                          <li className='leading-6'><Link to="/profile" onClick={toggleMenu} className="block hover:text-black hover:bg-gray-200 dark:hover:bg-gray-700 dark:hover:text-white duration-300 px-3 py-1"><PiUserCircleFill className='text-[0.9rem] inline mr-2 mb-[2px]'/>Profile</Link></li>
                          <li className='leading-6'><Link to="/add_blog" onClick={toggleMenu} className="block  hover:text-black hover:bg-gray-200 dark:hover:bg-gray-700 dark:hover:text-white duraton-300 px-3 py-1"><RiAddCircleFill className='text-[0.9rem] inline mr-2 mb-[2px]'/>Add Blog</Link></li>
                          <li className='leading-6'><Link to={`/blogs/author/${userData?._id}`} onClick={toggleMenu} className="block  hover:text-black hover:bg-gray-200 dark:hover:bg-gray-700 dark:hover:text-white duraton-300 px-3 py-1"><FaRectangleList className='text-[0.9rem] inline mr-2 mb-[2px]'/>My Blogs</Link></li>
                          <li className='leading-6'><Link onClick={handleSignOut} className="block  hover:text-black hover:bg-gray-200 dark:hover:bg-gray-700 dark:hover:text-white duration-300 px-3 py-1"><IoMdLogOut className='text-[0.9rem] inline mr-2 mb-[2px]' />Logout</Link></li>
                      </ul>
                  </div>
              </div>
            ...
            :  
              ...
              <div className='hidden md:block py-1'>
                <Button type='button' onClick={()=>navigate('/sign_in')} className='text-white bg-lightPrimary hover:bg-primary dark:bg-primary dark:hover:bg-lightPrimary duration-300'>Sign In</Button>
              </div>
            }
        ...
        {/* Mobile menu bar */}
          ...
          <div className={`md:hidden fixed inset-0 z-10 bg-white dark:bg-slate-800 pt-[0.55rem] px-[1rem] ${!isMenuOpen?'hidden':''}` }>            
              <div className='flex justify-between items-center gap-3 mb-3'>    
                  {/* logo */}
                  <div className='dark:brightness-[1.20] contrast-[0.85]'>
                    <Link to="/" onClick={toggleMenu}><img src={logo} className='w-[90px] h-auto max-w-[85px] md:max-w-[95px]' alt="logo" /></Link>
                  </div>
              ...
              <div className='pb-[0.8rem] mb-[1rem] border-b-2 border-gray-300'>
                  <ul>
                    <li className='leading-7 mt-1'><Link to="/" onClick={toggleMenu} className="block border-l-2 border-transparent rounded hover:border-primary hover:text-black dark:text-white hover:bg-slate-200 dark:hover:bg-gray-600 duration-300 px-3 py-1">Home</Link></li>
                    <li className='leading-7 mt-1'><Link to="/blogs" onClick={toggleMenu} className="block border-l-2 border-transparent rounded hover:border-primary hover:text-black dark:text-white hover:bg-slate-200 dark:hover:bg-gray-600 duration-300 px-3 py-1">All Blogs</Link></li>
                    <li className='leading-7 mt-1'><Link to="/about_us" onClick={toggleMenu} className="block border-l-2 border-transparent rounded hover:border-primary hover:text-black dark:text-white hover:bg-slate-200 dark:hover:bg-gray-600 duration-300 px-3 py-1">About</Link></li>
                 </ul>
              </div>
                  ...
                  <div>
                      {isAuthenticated?
                        <>
                          ...
                          // login-button
                            <div>
                              <Button type='button' onClick={()=>{navigate('/sign_in'),toggleMenu()}} className='text-white bg-lightPrimary hover:bg-primary duration-300 dark:bg-primary dark:hover:bg-lightPrimary'>Sign In</Button>
                            </div>
                        }
                  </div>
                  {/* themeButton */}
                  <div onClick={()=>dispatch(themeToggle())} className='border border-gray-500 rounded-full mr-4 p-[0.4rem] hover:bg-gray-200 dark:hover:bg-gray-700 duration-300 cursor-pointer'>
                                  {themeMode==="light"?<HiOutlineMoon className='text-[1rem] text-gray-800'/>
                                                      :<BiSun className='text-[1rem] text-gray-800 dark:text-white'/>
                                  }
                  </div>
